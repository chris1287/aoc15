...
pub fn fitness(a: &Sue, b: &Sue) -> u64 {
    let mut matching = 0;
    for ca in &a.compounds {
        matching += b.compounds.iter().filter(|cb|{
            match (ca, *cb) {
                (Compound::Cats(na), Compound::Cats(nb)) => na < nb,
                (Compound::Trees(na), Compound::Trees(nb)) => na < nb,
                (Compound::Pomeranians(na), Compound::Pomeranians(nb)) => na > nb,
                (Compound::Goldfish(na), Compound::Goldfish(nb)) => na > nb,
                (na, nb) => na == nb
            }
        }).count();
    }
