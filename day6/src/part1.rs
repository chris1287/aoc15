...
pub fn solve(s: &str) -> usize  {
    ...
    let mut grid = Grid::init(1000, 1000, false);
    for command in commands {
        for x in command.start.x..=command.end.x {
            for y in command.start.y..=command.end.y {
                match command.instruction {
                    Instruction::On => grid[(x, y)] = true,
                    Instruction::Off => grid[(x, y)] = false,
...
mod tests {
    ...
    #[test]
    fn test1() {
        assert_eq!(solve("turn on 0,0 through 999,999"), 1000 * 1000);
        assert_eq!(solve("turn on 0,0 through 999,0"), 1000);
    }

    #[test]
    fn test2() {
        let data = std::fs::read_to_string("input/input.txt").unwrap();
        assert_eq!(solve(&data), 569999);
    }
}
